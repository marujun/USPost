    ...
    try {
        ...
        */
        ...
        setupWebViewJavascriptBridge(function(bridge) {
            bridge.registerHandler('set_json', function(data, responseCallback) {
                console.log("set_json called with:", data);
                codeEditor.set(data);
                responseCallback({"success": true})
            });
            bridge.registerHandler('get_json', function(data, responseCallback) {
                console.log("get_json called with:", data);

                var jsontext = codeEditor.getText();
                if (!jsontext) codeEditor.set({});

                try { codeEditor.format(); }
                catch (err) { codeEditor._onError(err); }

                responseCallback(codeEditor.get());
            });
        });
